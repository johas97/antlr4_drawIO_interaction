...
from ParseTreeExtractor import ParseTreeExtractor
import drawpyo
from os import path

def main():
    #Read file input
    with open('python/example/meme.py') as file:
        file_data = f'{file.read()}'
        print(file_data)
        #Create ANTLR4 parser and parse tree based on file input
    ...
    #Create object that has parse tree data
    printable_tree = ParseTreeExtractor(tree)
    print(printable_tree)


    # Create file and start a flowchart
    file = drawpyo.File()
    file.file_path = path.join('generated', "Test Drawpyo from ANTLR4 charts")
    file.file_name = 'meme'
